# Fase 4 - Spring AI + OpenAI + PgVector
...
│ (usuario)│    │ Chain  │  SafeguardAdvisor)  │                         │    │ (OpenAI) │
└─────────┘    └────────┤                     │                         │    └──────────┘
                        └──────────┬──────────┴─────────────────────────┘
                                   │
                        ┌──────────▼──────────┐
...
- **Cuenta de OpenAI** y variable de entorno **`OPENAI_API_KEY`**
- Modelos por defecto (configurables en `application.yml`):
  - `gpt-4o-mini` (chat)
  - `text-embedding-3-small` (embeddings, **1536** dimensiones; deben coincidir con `spring.ai.vectorstore.pgvector.dimensions`)
...
```bash
cd fase4-spring-ai-openai-pgvector
docker compose up -d
```
...
podman run -d --name fase4-openai-pgvector \
  -e POSTGRES_DB=fase4_openai_ragdb \
  -e POSTGRES_USER=postgres \
  -e POSTGRES_PASSWORD=postgres \
  -p 5433:5432 \
  -v fase4_openai_pgvector_data:/var/lib/postgresql/data \
...
**Linux / macOS:**
```bash
docker exec fase4-openai-pgvector psql -U postgres -d fase4_openai_ragdb -c "SELECT 1;"
# o con podman:
podman exec fase4-openai-pgvector psql -U postgres -d fase4_openai_ragdb -c "SELECT 1;"
```
...
```bash
# Obligatorio: API key de OpenAI (no la subas a git)
export OPENAI_API_KEY="sk-..."

# Opcional: modelos (coinciden con los defaults de application.yml)
export OPENAI_CHAT_MODEL="gpt-4o-mini"
export OPENAI_EMBEDDING_MODEL="text-embedding-3-small"
```

Si la key no está definida, la aplicación fallará al arrancar al llamar a la API de OpenAI.
...
mvn clean install -pl fase4-spring-ai-openai-pgvector -am -DskipTests
...
|---|---|---|
| `OPENAI_API_KEY` | _(vacío)_ | **Obligatoria**. API key de OpenAI |
| `OPENAI_CHAT_MODEL` | `gpt-4o-mini` | Modelo de chat |
| `OPENAI_EMBEDDING_MODEL` | `text-embedding-3-small` | Modelo de embeddings (**1536** dims con el default) |
| `POSTGRES_HOST` | `localhost` | Host de PostgreSQL |
| `POSTGRES_PORT` | `5433` | Puerto (mapeado por `docker-compose` de esta demo) |
| `POSTGRES_DB` | `fase4_openai_ragdb` | Base de datos |
...
La tabla `vector_store` se crea automáticamente con `initialize-schema=true`. Usa índice **HNSW** con **distancia coseno** y dimensiones **1536** (compatible con `text-embedding-3-small` por defecto).

Para verificar la tabla creada:
```bash
# Docker
docker exec fase4-openai-pgvector psql -U postgres -d fase4_openai_ragdb -c "\dt"
docker exec fase4-openai-pgvector psql -U postgres -d fase4_openai_ragdb -c "SELECT count(*) FROM vector_store;"

# Podman
podman exec fase4-openai-pgvector psql -U postgres -d fase4_openai_ragdb -c "\dt"
podman exec fase4-openai-pgvector psql -U postgres -d fase4_openai_ragdb -c "SELECT count(*) FROM vector_store;"
```
...
# Ver logs
podman logs -f fase4-openai-pgvector

# Detener
podman stop fase4-openai-pgvector

# Eliminar contenedor
podman rm fase4-openai-pgvector

# Eliminar contenedor + volumen
podman rm fase4-openai-pgvector && podman volume rm fase4_openai_pgvector_data
```
...
| Problema | Solución |
|---|---|
| `Connection refused` en `/api/chat` o errores HTTP al llamar al modelo | Comprobar `OPENAI_API_KEY`, red saliente y cuotas de OpenAI |
| `Connection refused` en PgVector | Verificar contenedor en **puerto 5433**: `docker ps` o `podman ps` |
| `Table vector_store does not exist` | Reiniciar app (se crea con `initialize-schema=true`) |
| Embeddings de dimensión incorrecta | Alinear `OPENAI_EMBEDDING_MODEL`, dimensiones del modelo y `spring.ai.vectorstore.pgvector.dimensions` (p. ej. 1536 para `text-embedding-3-small`) |
| Puerto 5433 ocupado | Cambiar en `docker-compose.yml` y `application.yml`, o detener el otro PostgreSQL |
