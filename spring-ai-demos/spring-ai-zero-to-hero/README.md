...
**NO ejecutes** `mvn spring-boot:run` desde el directorio raíz sin especificar un proyecto, ya que Maven fallará con el error `Unable to find a suitable main class` (el proyecto raíz no tiene una clase principal).

Debes ejecutar las aplicaciones específicas usando el flag `-pl` (project list):
...
- **Puerto 8080 en uso**: Si ves el error `Port 8080 was already in use`, puedes cambiar el puerto al arrancar:
  ```bash
  ./mvnw spring-boot:run -pl applications/provider-ollama -Dspring-boot.run.arguments="--server.port=8081"
  ```
- **Falta de Main Class**: Asegúrate de NO ejecutar `mvn spring-boot:run` en la raíz. Usa siempre el flag `-pl`.
- **Ollama Connection Refused**: Si ves `ConnectException: I/O error on POST request for "http://localhost:11434/api/chat"`, significa que Ollama no está corriendo o el modelo no ha sido descargado.
  - Inicia la aplicación de escritorio de Ollama.
  - Descarga los modelos: `ollama pull llama3.2` y `ollama pull mxbai-embed-large`.
- **Dependencias No Encontradas**: Si ves `Could not resolve dependencies`, ejecuta `./mvnw clean install -DskipTests -U` desde la raíz. El flag `-U` es crucial para corregir errores de resolución cacheados.

## Software Prerequisites

**You need the following software installed: Java 21+, docker (or podman), ollama, httpie,
...
* [Docker](https://www.docker.com/products/docker-desktop) or [Podman](https://podman.io/) so we can use test containers & for local
  dependencies. If using Podman, you should also install `podman-compose`.
