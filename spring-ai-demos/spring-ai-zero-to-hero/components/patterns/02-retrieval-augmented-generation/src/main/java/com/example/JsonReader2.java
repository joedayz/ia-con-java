...
public class JsonReader2 implements DocumentReader {
  ...
  private Document parseJsonNode(JsonNode jsonNode, ObjectMapper objectMapper) {
    ...
    Map<String, Object> generatedMetadata = this.jsonMetadataGenerator.generate(item);
    Map<String, Object> metadata = new HashMap<>(generatedMetadata);
    // Add original ID or name to metadata for easier identification
    if (item.containsKey("name")) {
      metadata.put("source_name", item.get("name"));
    }
    if (item.containsKey("id")) {
      metadata.put("source_id", item.get("id"));
    }
