...
By default, this sample Spring AI app connects to OpenAI.

You can store only LLM tokens in `/Users/josediaz/.api-keys` (loaded automatically by Spring Boot):
```properties
OPEN_AI_TOKEN=<YOUR_API_TOKEN>
MISTRAL_AI_TOKEN=<YOUR_API_TOKEN>
ANTHROPIC_AI_TOKEN=<YOUR_API_TOKEN>
AZURE_OPENAI_API_KEY=<YOUR_AZURE_OPENAI_API_KEY>
DEEPSEEK_AI_TOKEN=<YOUR_API_TOKEN>
```

With this file in place, you can run without exporting LLM environment variables. Environment variables still work and override file values when both are present.

Configure non-LLM credentials directly in project properties:
- `src/main/resources/application.properties`
  - `app.vectorstore.type` (`simple` by default, use `pinecone` for Pinecone)
  - `app.stock.api-key`
  - `app.vectorstore.pinecone.api-key`
- `src/main/resources/application-azure-ai.properties`
  - `app.vectorstore.cosmosdb.key`

By default, `just run` starts with `app.vectorstore.type=simple` (passed explicitly in the run command), so the app can boot without Pinecone while you test chat endpoints like `/persons`.
When you want Pinecone-backed RAG, configure `app.vectorstore.pinecone.api-key` and run:
```shell
just run-rag
```

For local containers, the `just` recipes support both runtimes:
- on macOS with Podman: use `just containers-up` (auto-detects Podman first when available)
- for students using Docker: use `just containers-up engine=docker`

Useful container commands:
```shell
just containers-runtime
just containers-up
just containers-ps
just containers-down
```

If you prefer environment variables only, you can still run as before:
```shell
...
For scenarios with a vector store (`StockController`, `ImageController`) configure `app.vectorstore.pinecone.api-key` in `src/main/resources/application.properties`.

For scenarios with a stock API (`StockController`, `WalletController`) configure `app.stock.api-key` in `src/main/resources/application.properties`.
