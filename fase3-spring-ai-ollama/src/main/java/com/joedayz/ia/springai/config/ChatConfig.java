...
import com.joedayz.ia.springai.memory.JdbcChatMemoryRepository;
 ...
 *
 * Permite alternar entre memoria en RAM y memoria persistente en H2.
 */
@Configuration
public class ChatConfig {
    ...
    public ChatMemory chatMemory(DataSource dataSource, PlatformTransactionManager transactionManager) {
        ChatMemoryRepository repository = switch (repositoryType.trim().toLowerCase()) {
            case "jdbc", "persistent", "persistente", "h2" -> new JdbcChatMemoryRepository(dataSource,
                    transactionManager);
        ...
        if (repository instanceof InMemoryChatMemoryRepository) {
            ...
            System.out.println("   💾 La memoria persistirá en la base H2 configurada");
        }
