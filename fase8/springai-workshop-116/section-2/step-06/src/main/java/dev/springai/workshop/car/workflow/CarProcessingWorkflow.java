...
/**
 * Cadena con {@link ChainWorkflow}: imagen → análisis paralelo → supervisor → condición.
 */
@Service
public class CarProcessingWorkflow {
    ...
    public CarConditions processCarReturn(
        ...
        FleetProcessingContext result = chainWorkflow.chainSteps(
                FleetProcessingContext.initial(tasks, carInfo, carNumber, feedback, carImage),
                List.of(
                        this::enrichFeedbackFromImage,
    ...
    private FleetProcessingContext enrichFeedbackFromImage(FleetProcessingContext ctx) {
        String enriched = carImageAnalysisAgent.analyzeCarImage(ctx.feedback(), ctx.carImage());
        log.debug("Enriched feedback: {}", enriched);
        return ctx.withWorkingFeedback(enriched);
    }
    ...
    private FleetProcessingContext runParallelAnalysis(FleetProcessingContext ctx) {
        var analysis = feedbackAnalysisWorkflow.analyzeFeedback(
                ctx.tasks(), ctx.carInfo(), ctx.carNumber(), ctx.workingFeedback());
    ...
    private FleetProcessingContext runSupervisor(FleetProcessingContext ctx) {
        String decision = fleetSupervisorAgent.superviseCarProcessing(
                ctx.carInfo(), ctx.carNumber(), ctx.workingFeedback(), ctx.analysisResults());
