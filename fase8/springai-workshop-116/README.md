# springai-workshop-116

Workshop para construir aplicaciones con IA usando **Spring Boot** y **Spring AI 1.1.6**, con **Anthropic (Claude)** como proveedor por defecto.

Réplica de [springai-workshop](../springai-workshop/) (Spring AI **1.0.5**) con los mismos pasos y dominio, actualizada a la API de memoria de chat de 1.1.6 (`ChatMemory.CONVERSATION_ID` en el contexto del advisor).
...
### Cambios respecto a springai-workshop (1.0.5)

- BOM y dependencias: `spring-ai.version=1.1.6`
- Memoria de chat: el ID de conversación se pasa con `.advisors(a -> a.param(ChatMemory.CONVERSATION_ID, sessionId))` (ya no `.conversationId()` en el builder del advisor)
- RAG: `TokenTextSplitter` usa el patrón `TokenTextSplitter.builder()...build()` en lugar del constructor de 5 argumentos
- **Section 2 (steps 02–07)**: workflows con patrones de [Building Effective Agents](https://docs.spring.io/spring-ai/reference/api/effective-agents.html) vía módulo `agentic-patterns` (`ChainWorkflow`, `ParallelizationWorkflow`, `RoutingWorkflow`), portados de [spring-ai-examples/agentic-patterns](https://github.com/spring-projects/spring-ai-examples/tree/main/agentic-patterns)
...
| `section-2/step-02` | `ParallelizationWorkflow`: limpieza + condición en paralelo |
| `section-2/step-03` | Chain + Parallelization + Routing (`ChainWorkflow`, `ParallelizationWorkflow`, `RoutingWorkflow`) |
...
| `@ParallelAgent` | `ParallelizationWorkflow.parallelExecute` (steps 02–07) |
| `@SequenceAgent` | `ChainWorkflow.chainSteps` (steps 03–07) |
| `@ConditionalAgent` | `RoutingWorkflow.selectRoute` + agentes (step-03; supervisor en step-04+) |
