...
/**
 * RAG con Ollama + PgVector:
 * 1) recupera fragmentos similares de PgVector,
 * 2) los inyecta en el prompt,
 * 3) genera una respuesta con citas.
 *
 * Soporta tanto RAG manual como RAG con QuestionAnswerAdvisor.
 */
@Service
public class RagService {
    ...
    public RagResponse answer(RagRequest request) {
        ...
        if (contextDocs.isEmpty()) {
            return new RagResponse(
                    question,
                    "No encontré contexto suficiente en PgVector. Primero indexa documentos con /api/buscar/demo, /api/buscar/pdf o /api/rag/docs/cargar.",
                    topK,
                    false,
                    List.of());
        }
    ...
    public Map<String, Object> cargarDocumentosMarkdown(String directoryPath) {
        ...
        for (File mdFile : mdFiles) {
            int chunks = semanticSearchService.cargarMarkdown(mdFile.getAbsolutePath(), mdFile.getName());
            totalChunks += chunks;
            archivos.add(mdFile.getName() + " (" + chunks + " chunks)");
        }

        return Map.of(
                "message", "Documentos Markdown indexados en PgVector",
                "archivos", archivos,
